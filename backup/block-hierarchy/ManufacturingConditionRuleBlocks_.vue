...
const isSpecChecked = (blkIndex, specCode) =>  {localBlockContents.value[blkIndex].data[0].metadata?.programs?.some(specInfo => specInfo.specName == specCode); }
const onToggleSpec = async (blkIndex, opt) => {
  ...
  if (existsIdx >= 0) {
    ...
    localBlockContents.value[blkIndex].data[0].metadata.programs.splice(existsIdx);
  }
