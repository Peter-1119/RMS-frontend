      ...
      <div v-if="currentStep !== 9" class="content-page">
        ...
        <section :ref="el => sectionRefs[3].value = el" class="step-section">
          ...
          <div class="Management">
            <button class="layer-action-btn add" @click="addDynamicBlock('management', 1)">新增下一層</button>
          </div>
          ...
          <div class="management-combination-block">
            <ManagementSpecificBlock
              ...
              :step="3"
              :tier="1"
              :blockContent="blocks?.management[0]?.data[0]"
              :allow-color="isRevisionDoc"
              @update-block="blocks.management[0].data[0] = $event"
              @invalid-check="(payload) => updateDynamicValidation(3, 'management', 'main', payload)"
            />
          </div>
          <div v-if="blocks.management.length > 1" class="management-content-bloc">
            <DynamicEditorBlock
              v-for="(blk, blkIndex) in blocks.management.slice(1)"
              :key="blk.id"
              :step="3"
              :tier="blkIndex + 2"
              :blockContent="blk.data"
              :allow-color="isRevisionDoc"
              @add-block="addDynamicBlock('management', 1)"
              @delete-block="removeDynamicBlock('management', blkIndex + 1)"
              @update-block="blocks.management[blkIndex + 1].data = $event"
              @invalid-check="(payload) => updateDynamicValidation(3, 'management', blk.id, payload)"
            />
          </div>
        </section>
        ...
        <section :ref="el => sectionRefs[5].value = el" class="step-section">
          <h2>5. 異常處置</h2>
          <div class="Exception">
            <button class="layer-action-btn add" @click="addDynamicBlock('exception', 3)">新增下一層</button>
          </div>

          <DynamicEditorBlock
            v-for="(blk, blkIndex) in blocks.exception"
            :ref="el => setExceptionEditorRef(blk.id, el)"
            :key="blk.id"
            :step="5"
            :tier="blkIndex + 1"
            :blockContent="blk.data"
            :allow-color="isRevisionDoc"
            @add-block="addDynamicBlock('exception', 3)"
            @delete-block="removeDynamicBlock('exception', blkIndex)"
            @update-block="blocks.exception[blkIndex].data = $event"
            @invalid-check="(payload) => updateDynamicValidation(5, 'exception', blk.id, payload)"
          ></DynamicEditorBlock>
        </section>
...
const blocks = reactive({ processFlow: null, management: [], manufacture: [], exception: [] });
...
const stepStatusClass = (stepNo) => {
  ...
  if (stepNo == 3 && blocks.management.length == 1 && blocks.management[0].data[0].table_json == null) return '';
...
const buildDocxPayload = () => {
  return {
    ...
    form_attribute: { document_name: formJson.document_name, applyProject: formJson.applyProject, purpose: formJson.purpose },
    content: [...normalizeBlock([blocks.processFlow]), ...normalizeBlock(blocks.management), ...normalizeBlock(blocks.manufacture), ...normalizeBlock(blocks.exception)],
...
const collectInvalidSteps = () => {
  ...
  for (const [key, stepInfo] of Object.entries(stepValidations)) {
    ...
    if (n === 3 && blocks.management.length === 1 && blocks.management[0].data[0].table_json == null) continue;
...
const applyLoadedData = async () => {
  ...
  try {
    ...
    if (result.form){  // Load data
      ...
      if (result.blocks.processFlow) Object.assign(blocks.processFlow, { ...result.blocks.processFlow });
      if (result.blocks.management.length > 0) blocks.management = [...result.blocks.management];
      if (result.blocks.manufacture.length > 0) blocks.manufacture = [...result.blocks.manufacture];
      if (result.blocks.exception.length > 0) blocks.exception = [...result.blocks.exception];
      Object.assign(references, { ...result.references });

      if (form.attribute?.inputMachines?.length > 0) machine_tags.value = form.attribute.inputMachines.split(",").map(machineInfo => ({ id: machineInfo.slice(1, 7), name: machineInfo, isValid: true }));
      await machinesMatchCheck();
      await nextTick();

      // Parse data format through reverse tiptap table
      oldPmsTable = blocks.management[0].data[0].table_text;
      ...
      ManagementBlockComponent.value.initFromProps(blocks.management[0].tier_no == 1 ? blocks.management[0].data[0] : null);
