    ...
    <div class="form-section">
      ...
      <div v-if="currentStep !== 8" class="content-page">
        ...
        <section :ref="el => (sectionRefs[1].value = el)" class="step-section">
          <h2>1. 目的</h2>
          <div class="purpose-group"><textarea placeholder="此處將填寫文件的目的相關內容" v-model="form.purpose"></textarea></div>
        </section>

        <!-- Step 3 製作條件規範 -->
        <section :ref="el => (sectionRefs[2].value = el)" class="step-section">
          <h2>2. 製作條件規範</h2>
          <div class="manufacturing-specification-block"><button class="layer-action-btn add" @click="addDynamicBlock('production', 4)">新增下一層</button></div>

          <DynamicEditorBlock
            v-for="(blk, blkIndex) in blocks.production"
            :key="blk.id"
            :step="2"
            :tier="blkIndex + 1"
            :blockContent="blk.data"
            :allow-color="isRevisionDoc"
            @add-block="addDynamicBlock('production', 6)"
            @delete-block="removeDynamicBlock('production', blkIndex)"
            @update-block="blocks.production[blkIndex].data = $event"
            @invalid-check="(payload) => updateDynamicValidation(2, 'production', blk.id, payload)"
          ></DynamicEditorBlock>
        </section>
        ...
        <section :ref="el => (sectionRefs[4].value = el)" class="step-section">
          <h2>4. 適用品質與規格內容</h2>
          <div class="quality-specification-block"><button class="layer-action-btn add" @click="addDynamicBlock('quality', 6)">新增下一層</button></div>
          <DynamicEditorBlock
            ref="qualityBlocksRef"
            v-for="(blk, blkIndex) in blocks.quality"
            :key="blk.id"
            :step="4"
            :tier="blkIndex + 1"
            :blockContent="blk.data"
            :allow-color="isRevisionDoc"
            :documentMode="true"
            @open-doc-search="handleOpenDocSearch(blkIndex)"
            @add-block="addDynamicBlock('quality', 6)"
            @delete-block="removeDynamicBlock('quality', blkIndex)"
            @update-block="blocks.quality[blkIndex].data = $event"
            @invalid-check="(payload) => updateDynamicValidation(4, 'quality', blk.id, payload)"
          ></DynamicEditorBlock>
        ...
        <section :ref="el => (sectionRefs[6].value = el)" class="step-section">
          <h2>6. 其它</h2>
          <div class="other-block"><button class="layer-action-btn add" @click="addDynamicBlock('other', 7)">新增下一層</button></div>
          <DynamicEditorBlock
            v-for="(blk, blkIndex) in blocks.other"
            :key="blk.id"
            :step="6"
            :tier="blkIndex + 1"
            :blockContent="blk.data"
            :allow-color="isRevisionDoc"
            @add-block="addDynamicBlock('other', 7)"
            @delete-block="removeDynamicBlock('other', blkIndex)"
            @update-block="blocks.other[blkIndex].data = $event"
            @invalid-check="(payload) => updateDynamicValidation(6, 'other', blk.id, payload)"
          ></DynamicEditorBlock>
        </section>
      </div>
...
import DynamicEditorBlock from '@/components/DynamicEditorBlock_.vue'
...
const handleOpenDocSearch = (index) => {
  insertIndex.value = index;
  specDocWindowVisible.value = true;
};
const createQualityDocBlock = (docInfo) => ({ 
  content_type: 3,
  header_json: { type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: docInfo }] }] }, 
  header_text: docInfo, 
  content_json: null, 
  content_text: null, 
  table_json: null, 
...
const qualityBlocksRef = ref([]);
const addQualityDocs = (payload) => {
  console.log("blocks.quality: ", blocks.quality);
  // console.log("insertIndex: ", insertIndex.value);
  payload.forEach((doc, index) => {
    // blocks.quality.splice(insertIndex.value, 1, { id: uuid++, step_type: 6, tier_no: 1, data: [createQualityDocBlock(`${doc.refer_document} ${doc.refer_document_name}`)] });
    if (index == 0) { Object.assign(blocks.quality[insertIndex.value], { data: [createQualityDocBlock(`${doc.refer_document} ${doc.refer_document_name}`)] }); }
    else { blocks.quality.splice(insertIndex.value, 0, { id: uuid++, step_type: 6, tier_no: 1, data: [createQualityDocBlock(`${doc.refer_document} ${doc.refer_document_name}`)] }); }
    insertIndex.value += 1;
  })
  blocks.quality.forEach((blk, blkIndex) => { blk.tier_no = blkIndex + 1; });
  specDocWindowVisible.value = false;
  insertIndex.value = -1;
  console.log("blocks.quality: ", blocks.quality);

  nextTick(() => {
    if (qualityBlocksRef.value) {
      qualityBlocksRef.value.forEach(comp => {
        if (comp && comp.loadBlockEditor) {
          comp.loadBlockEditor();
        }
      });
    }
  });
...
const addDocs = (payload) => {
  const defaultColor = isRevisionDoc.value ? 'blue' : 'black';
...
const buildDocxPayload = () => {
  return {
    ...
    attribute: [{ ...form }],
    content: [...normalizeBlock(blocks.production), ...normalizeBlock(blocks.manufacture), ...normalizeBlock(blocks.quality), ...normalizeBlock(blocks.other)],
...
const applyLoadedData = async () => {
  ...
  try {
    ...
    if (result.form){  // Load data
      ...
      if (result.blocks.production) Object.assign(blocks.production, { ...result.blocks.production });
      if (result.blocks.manufacture.length > 0) { blocks.manufacture = [...result.blocks.manufacture]; };
      if (result.blocks.quality.length > 0) blocks.quality = [...result.blocks.quality];
      if (result.blocks.other.length > 0) blocks.other = [...result.blocks.other];
      Object.assign(references, { ...result.references });
    }

    if (result.blocks.manufacture.length > 0) {
...
const unMountedFunction = async () => {
  window.removeEventListener('scroll', handleScroll);
  if (confirm('請問是否要保存內容')) await saveDraft();
  resetAll();
  clearToken();
}
