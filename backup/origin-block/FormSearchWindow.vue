    ...
    <div class="dialog">
      ...
      <div class="content" style="position: relative;"> <div v-if="loading" class="loading-overlay">
        ...
        <table class="search-table">
          ...
          <tbody>
            <tr v-for="formInfo in formInfos" :key="formInfo.formId" 
...
export default{
  ...
  methods: {
    ...
    toggleSelect(formInfo) {
      const idx = this.selectedList.findIndex(f => f.formId === formInfo.formId);
      if (idx > -1) {
        this.selectedList.splice(idx, 1); // 已經選過，再次點擊就移除
      } else {
        this.selectedList.push(formInfo); // 沒選過，加入陣列
      }
    },
    
    // ★ 修改點：判斷是否已被選取
    isSelected(formInfo) {
      return this.selectedList.some(f => f.formId === formInfo.formId);
    },

    // ★ 修改點：確認送出，直接送出整個 Array
    addNewForms() {
      if (this.selectedList.length === 0){
        alert("請至少選擇一筆資料");
        return;
      }
      // 改名為 add-new-forms 傳送出去
