...
const CustomTableCell = TableCell.extend({
  ...
  addNodeView() {
    return ({ node, getPos, editor }) => {
      ...
      // ⭐ 自訂 nodeView 繞過了預設 td 渲染，必須手動把 colspan/rowspan 套到 DOM，
      //    否則合併(跨列)不會顯示、被移除的格子會「消失」(model 有 rowspan，畫面卻沒套)。
      if (node.attrs.colspan && node.attrs.colspan !== 1) td.colSpan = node.attrs.colspan;
      if (node.attrs.rowspan && node.attrs.rowspan !== 1) td.rowSpan = node.attrs.rowspan;
