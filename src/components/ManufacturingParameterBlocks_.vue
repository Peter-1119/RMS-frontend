...
const isMachineDisabled = (blkIndex, machineCode) => {
  ...
  const matchSet = machineMatchSet[blk.id];
  const selected = blk.data[0].metadata.machines || [];
  // 已選取的機台永遠可操作（即使變成 conflict 也要能取消糾錯）；
  // 只 disable「未選取且不在 matchSet」的機台，避免加入不相容的新機台。
  return matchSet.size > 0 && !matchSet.has(machineCode) && !selected.includes(machineCode);
}
