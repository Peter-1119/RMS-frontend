...
export async function getEngineeringProcesses(projectId, keyword) {
//   const { data } = await axios.get(`${API}/mes/engineering/${encodeURIComponent(projectId)}/processes`);
  const { data } = await axios.get(`${API}/mes/engineering/processes`, { params: { project_id: projectId, keyword }});
  return data?.data || []
}

export async function getUnassignedProcesses({ projectCode, keyword='', page=1, pageSize=20 }) {
  const { data } = await axios.get(`${API}/mes/engineering/unassigned-processes`, { params: { keyword, page, pageSize } });
  return data?.data || { items: [], total: 0, page, pageSize };
}
